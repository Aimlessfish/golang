...
// SessionManager manages multiple Steam bot sessions
type SessionManager struct {
	...
	credStore     *util.CredentialStore
}

// NewSessionManager creates a new session manager
func NewSessionManager(startPort int) *SessionManager {
	// Initialize credential store
	credStore, err := util.NewCredentialStore("steam_credentials.json")
	if err != nil {
...
func (sm *SessionManager) AddSession(userID string, profileURL string) (*Session, error) {
	...
	if sm.credStore != nil {
		...
		if err == nil {
			...
			session = NewSessionWithCredentials(id, userID, port, debugPort, cred)
		} else {
...
func (sm *SessionManager) AddCredential(cred *util.SteamCredential) error {
	if sm.credStore == nil {
